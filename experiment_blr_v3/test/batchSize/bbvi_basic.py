...
import os
'''
use bbvi_cv to test the influence of batchSize
'''
num_epochs=30
batchSize=500
num_S=5#训练的采样数量
...
for epoch in range(num_epochs):
    for i ,data in enumerate(train_loader):
        ...
        print(torch.max(rho)*torch.max(grad_avg))
        #print information
        if 1:
...
np.save('./test/batchSize/elbos3000.npy',result)
...
np.save('./test/batchSize/paras3000',result)