...
@torch.no_grad()
def data_preprocess(train_datas,data_index,dim):
    train_labels=[]
    train_features=[]
    index=data_index.clone().detach().numpy().astype(np.int)
    datas=np.array(train_datas)[index]
    '''
    转换格式
    '''
    ...
    return torch.Tensor(train_data[:,0]), torch.Tensor(train_data[:,1:])
