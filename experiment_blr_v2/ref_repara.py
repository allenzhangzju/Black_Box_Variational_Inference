...
def elbo_repara(images,labels,para,num_S,dim,scale):
    mu=para[0:dim]
    ...
    for i in range(num_S):
        ...
        elbo[i]=log_likelihood+log_prior/scale-log_q/scale
    elbo_avg=elbo.mean()
    return elbo_avg
'''
ref repara
...
for epoch in range(num_epochs):
    for i ,data in enumerate(train_loader):
        ...
        #算法起始位置
        grad_avg=torch.autograd.grad(elbo_repara(images,labels,para,num_S,dim,scale),para)[0]
...
np.save('./result/ref_repara.npy',result)
